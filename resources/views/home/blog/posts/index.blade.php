                  ...
                  <tbody>
                    @foreach ($data as $item)
                      <tr>
                        ...
                        <td>
                          <span class="badge {{ $item->visit < 20 ? 'bg-danger' : ($item->visit <= 80 ? 'bg-warning' : 'bg-success') }}">
                            {{ $item->visit ?? 0 }}
                          </span>
                        </td>

                        <td>
                          <span class="badge {{ $item->publish_status == true ? 'bg-success' : 'bg-danger' }}">
                            {{ $item->publish_status == true ? 'Published' : 'Darft' }}
                          </span>
                        </td>

                        <td>
                          <div class="btn-group btn-group-sm gap-1" role="group" aria-label="Button action group">
                            <form action="{{ url('/blog/post/show') . '/' . $item->id }}" method="GET">
                              @csrf
                            ...
                            <form action="{{ url('/blog/post/edit') . '/' . $item->id }}" method="GET">
                              @csrf
                            ...
                            <form action="{{ url('/blog/post/destroy') . '/' . $item->id }}" method="POST">
                              @method('DELETE')
                              @csrf
